...
CreateConVar("tttws_fire_fallback", "0", FCVAR_ARCHIVE)
...
function ENT:Initialize()
   ...
   if CLIENT and GetConVar("tttws_fire_fallback"):GetBool() then
...
function StartFires(pos, tr, num, lifetime, explode, dmgowner)
   for i=1, num do
      ...
      local flame = ents.Create("tttws_flame")
      flame:SetPos(pos)
...
function ENT:Think()
   ...
   if self.dietime < CurTime() then
      if self:GetExplodeOnDeath() then
         ...
         if not success then
            ErrorNoHalt("ERROR CAUGHT: tttws_flame: " .. err .. "\n")
         end
      end
