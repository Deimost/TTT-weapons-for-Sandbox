...
if CLIENT then
   SWEP.PrintName            = "#tttwfs_crowbar_name"
   SWEP.Slot               = 0
   SWEP.SlotPos            = 5
   ...
   SWEP.IconOverride       = "vgui/entities/tttws_improvised.png"
end
...
function SWEP:DrawWeaponSelection( x, y, wide, tall, alpha )
   ...
   draw.SimpleText("c", "HL2SelectIcons", xPos, yPos, Color( 255, 250, 0, 255 ), TEXT_ALIGN_CENTER )
   draw.SimpleText("c", "HL2SelectIcons2", xPos, yPos, Color( 255, 250, 0, 255 ), TEXT_ALIGN_CENTER )
end
...
if SERVER then
   CreateConVar("tttws_crowbar_unlocks", "1", FCVAR_ARCHIVE)
   CreateConVar("tttws_crowbar_pushforce", "395", FCVAR_NOTIFY)
end
...
function SWEP:OpenEnt(hitEnt)
   ...
   if SERVER and GetConVar("tttws_crowbar_unlocks"):GetBool() then
...
function SWEP:SecondaryAttack()
   ...
   if tr.Hit and IsValid(tr.Entity) and tr.Entity:IsPlayer() and (self:GetOwner():EyePos() - tr.HitPos):Length() < 100 then
      ...
      if SERVER and (not ply:IsFrozen()) then
         local pushvel = tr.Normal * GetConVar("tttws_crowbar_pushforce"):GetFloat()
